---
description: End-of-session orchestrator — sync from base → test → open PR → verify gate → review → fix → merge to main → handover. Runs the whole loop; merges only on a clean green path, and won't spend the review fan-out on work nobody has exercised.
...
| `no-sync` | Skip the Phase 0.5 base-sync and ship the branch against its fork point |
| *(empty)* | The full lap — sync → test → gate → review → fix → merge → handover |

**Any command also takes** `small`·`medium`·`large` (session size) · `terse`·`verbose` (how much Relay narrates) · `plain`·`informed`·`expert` (terminal depth) · `ask`·`challenge`·`solo` (who decides) — per-call, winning over `relay.config.local.json` ([[conventions]]). **Reads config:** `persist.cadence`, `tidy.level`, `hooks.test`, `hooks.release`, `hooks.env.down`, `hooks.affects`.
...
## Phase 0.5 — Sync from base if it moved (bracket the expensive middle)
The base-currency check isn't only the merge-gate's concern (Phase 5) — it's a **bracket** around the
costly middle of this loop. Telemetry from these repos is unambiguous: `main` moves on most laps
(median ~47 min between merges; on a busy day, 26 of them), and a single lap has been seen to absorb
*"22, then 10, then 4 commits"* of drift while it ran. Discovering that drift only at Phase 5 means the
review fan-out reviewed a **stale** diff, the merge then lands code **no reviewer saw**, and you pay a
full-suite re-verify at the finish line. Syncing here absorbs the drift that already existed *before*
this lap, so the review reads the true merged code and Phase 5's usual job collapses to the cheap
disjoint no-op. It does **not** replace Phase 5: `main` keeps moving *during* the loop, and that
residual is Phase 5's to catch — the check runs at both ends of the bracket, not one.

**`no-sync` in `$ARGUMENTS` skips this phase** (ship the branch as-is against its fork point).

1. **Resolve the base and whether it moved.** `<base>` = the default branch (`main`/`master`).
   ```bash
   git fetch origin
   BEHIND="$(git rev-list --count HEAD..origin/<base>)"
   ```
   `0` ⇒ the base hasn't moved since you forked — **say so in one line and go to Phase 1.** This is the
   common quiet case and costs nothing.

2. **It moved ⇒ decide whether it can reach you, the SAME predicate Phase 5 uses.** Run `hooks.affects`
   as a shell command line with two range arguments — `$1` = the base delta
   `$(git merge-base HEAD origin/<base>)..origin/<base>` (everything on base since you forked), `$2` =
   your work `origin/<base>...HEAD` — and read the verdict from **stdout + exit status**, never judge
   overlap yourself ([[conventions]] → Hooks; it's the identical deterministic predicate as Phase 5).
   - **`disjoint` + exit 0 ⇒ skip the sync.** What landed can't reach this PR's build or tests, so
     merging it in changes nothing this lap reviews. Say why in one line and go to Phase 1.
   - **Anything else** (`overlap`, a non-zero exit, an empty/unrecognised answer) ⇒ sync (step 3).
   - **No `hooks.affects` configured ⇒ sync unconditionally whenever the base moved.** This is the
     deliberate *opposite* of Phase 5's no-hook default, and the asymmetry is the whole point: Phase 5
     guards an **extra** full-suite re-run, so with no predicate to judge overlap it refuses to guess.
     Here there is no extra run — CI runs once in Phase 2 either way; the sync only decides *which tree*
     it runs against. A needless clean merge is cheap; reviewing stale code is not. So the additive
     default flips to "sync."

3. **Sync by MERGE, never rebase.** By now a PR often exists with pushed commits, review threads, and
   Phase 2.5 test-drive evidence anchored to those commits. A **rebase force-pushes and orphans all of
   it** — the one operation this loop must never do to its own PR. A merge is additive and keeps every
   ref intact.
   - **Dirty tree first:** if `git status` shows uncommitted work, commit the slice exactly as Phase 2
     step 2 does (Conventional-Commits, from the real diff, never asks) so the merge has a clean base.
   - **Then bring the base in:** if an OPEN PR already exists, `gh pr update-branch <n>` (server-side,
     no force-push) then `git pull --ff-only`; otherwise `git merge --no-edit origin/<base>` locally.
   - **Conflict ⇒ STOP and hand it to the user to resolve now.** This is exactly *why* the sync lives
     here: the conflict surfaces **before** the review spend, not at the finish line after it. Report
     the conflicting paths and stop.

4. **Clean sync ⇒ continue to Phase 1**, which now tests the merged tree. Say in one line what you
   absorbed — e.g. *"merged origin/main (11 commits, incl. migration 0159) cleanly; testing the merged
   tree."*
...
- **The green still holds against the CURRENT base.** Phase 0.5 already absorbed the drift that
  existed *before* this lap, so what this catches is the residual — whatever siblings merged *during*
  the loop. `OLD=$(git rev-parse origin/<base>); git fetch origin; git rev-list --count HEAD..origin/<base>` — if it's `0` the base hasn't moved and the green stands. If it's not `0`, a sibling session merged first, so the green *might* no longer describe the merged result. **Whether it still does is a question of overlap** — did what landed touch anything this PR's build and tests depend on? — and only the project knows its own dependency graph, so Relay asks the project rather than guessing:
  - **With `hooks.affects` configured** ([[conventions]] → Hooks), **run its value as a shell command line** — so a multi-word value like `node scripts/relay/affects.mjs` works — appending **two git range expressions as arguments**: `$1` = the **base delta** `$OLD..origin/<base>` (what landed since your green), `$2` = the **PR diff** `origin/<base>...HEAD`. Unlike the action hooks, this is a **deterministic predicate, not an LLM skill**: read the verdict from its **stdout + exit status**, never judge overlap yourself — a boolean safety gate must be reproducible. It runs in the repo with those refs in hand, so it resolves each side to its own affected scope itself (`git diff --name-only <range>`, a `pnpm --filter '...[<ref>]'` closure, whatever fits the stack) — Relay passes ranges, not pre-computed file lists. It prints `disjoint` **and exits 0** when the base change cannot affect this PR's result; **anything else means `overlap`**. Its contract: report `overlap` for anything that invalidates *every* package — a repo-root / CI-config / lockfile change, a DB migration — not only for a package-graph intersection.
    - **A clean `disjoint` + exit 0 ⇒ merge on the existing green, no re-run.** Say in one line why it's safe — what landed, and why it can't reach this PR.
    - **Anything else ⇒ re-verify:** `overlap`, a non-zero exit, an error, or an empty/unrecognised answer all fall back to re-verify — update the branch, push, and wait for checks again before merging. Only an unambiguous disjoint verdict earns the skip; ambiguity never does.
  - **No `hooks.affects` ⇒ re-verify unconditionally, exactly as before** — update the branch, push, wait for checks. The optimisation is strictly opt-in; a wrong `disjoint` is the one failure mode that matters, so with no project hook to judge overlap Relay never guesses it.

  Why skipping the re-run is safe and not a corner cut: where the project runs the **full suite on `main` after every merge** — a standing convention in these repos — that post-merge run is the real backstop for a bad *combination*, and the per-PR re-verify is belt-and-braces that catches nothing for a genuinely disjoint change. If a disjoint merge ever did surprise the post-merge run it surfaces as a red `main`, the same signal the team already acts on, not a silent bad merge. This is the **poor-man's merge queue**: optimistic merge, gated on real overlap, backstopped by the always-on `main` run — paired with a serialised release, it stands in for the merge queue a private Team-plan repo can't have. This is still the condition that makes parallel sessions safe to merge without coordinating: whoever gets there second re-verifies **whenever it could matter**.
