...
## Usage
`/relay:next [track|theme|slug] [verify|audit] [ask|challenge|solo]` — also `/rln` (bare, no prefix) and `/relay:rln`

| Argument | Effect |
|---|---|
| `<track>` · `<theme>` · `<slug>` | Bias the shortlist, or start that exact item |
| `verify` (`check`, `thorough`, `deep`) | L2 — rebuild the shortlist from the board's evidence |
| `audit` (`archive`, `exhaustive`, `compact`, `full`) | L3 — exhaustive pass over every item, plus archival |
| `ask` · `challenge` · `solo` | Who decides in this session (autonomy), for this run |
| *(empty)* | L1 — quick ranked shortlist |
...
4. **Carry `DECIDE` into the build** (a per-call `ask`/`challenge`/`solo` word in `$ARGUMENTS` wins —
   that's how two tabs run side by side at different levels). For a decision that will outlive the lap — the shape of stored
