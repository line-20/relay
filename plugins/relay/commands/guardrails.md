...
## Usage
`/relay:guardrails [dimension]`

| Argument | Effect |
|---|---|
| `<dimension>` | (Re)configure one — `api`, `ui`, `security`, `privacy`, `testing`, … |
| *(empty)* | The full per-dimension pass |

**Any command also takes** `small`·`medium`·`large` (session size) · `terse`·`verbose` (how much Relay narrates) · `plain`·`informed`·`expert` (terminal depth) · `ask`·`challenge`·`solo` (who decides) — per-call, winning over `relay.config.local.json` ([[conventions]]). **Reads config:** `guardrails.*`, `paths.guardrails`.
