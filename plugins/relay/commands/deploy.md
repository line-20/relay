...
## Usage
`/relay:deploy [pr-number]`

| Argument | Effect |
|---|---|
| `<pr-number>` | The PR whose preview to orchestrate and gate |
| *(empty)* | The current branch's PR |

**Any command also takes** `small`·`medium`·`large` (session size) · `terse`·`verbose` (how much Relay narrates) · `plain`·`informed`·`expert` (terminal depth) · `ask`·`challenge`·`solo` (who decides) — per-call, winning over `relay.config.local.json` ([[conventions]]). **Reads config:** `hooks.deploy`.
