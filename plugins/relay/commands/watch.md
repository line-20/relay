...
## Usage
`/relay:watch [pr-number | slug | branch]`

| Argument | Effect |
|---|---|
| `#<pr-number>` | Wait for that PR to land |
| `<track/slug>` | Wait for that board item |
| `<branch>` | Wait for that branch to reach `main` |
| *(empty)* | Asks what to wait for |
