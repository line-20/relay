...
## Usage
`/relay:refine [track/slug] [small|medium|large]` — also `/rlrf` (bare, no prefix) and `/relay:rlrf`

| Argument | Effect |
|---|---|
| `<track/slug>` | The brief to refine |
| `small` · `medium` · `large` | How big a slice it cuts (session size) |
| *(empty)* | Pick from the board |
