...
## Usage
`/relay:cross-check [topic | brief-slug] [conceptual|technical]`

| Argument | Effect |
|---|---|
| `<topic>` / `<design statement>` | Frame the reference set around this directly |
| `<brief slug>` | Read `<root>/briefs/<slug>.md` and check its Approach section |
| `conceptual` / `technical` | Aim the lens — problem framing vs. implementation prior art |
