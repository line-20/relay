...
The E2 gap — the checkpoint "lived on-disk in the worktree, resolvable but uncommitted" — is closed. `apply_pending` now, when given a repo and replication is on, commits each newly-applied checkpoint (with its completion marker, projection handover, and the board) to the **durable branch** and pushes it (`_replicate_checkpoint`, `scripts/relay_harvest.py`). The on-disk checkpoint is the offline floor; the push is best-effort and **never fails the apply** (R1.6's local-vs-remote split). A per-item `replicated` view (`_checkpoint_on_remote`, blob-hash comparison against the local `origin/<durable>` ref — no network) is surfaced through `discover_active`, so the maintainer can see which threads are resumable from another device.
