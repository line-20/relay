...
**Not yet wired into a command's happy path.** The runtime is git-durable; invoking `apply` (with replication) from the lifecycle — e.g. `/handover` or `/ship` — remains the manual-harness step E2 noted. Wiring it is a separate slice.

**Proven by** `CheckpointDurabilityTest` (`scripts/tests/test_relay_harvest.py`, real bare origin): recoverable from git after the worktree is removed; resumable from a fresh clone (the cross-device proof); offline push is non-fatal (local floor holds); replication refuses a topic branch; idempotent re-apply makes no second commit; `--no-replicate` writes state only; `discover_active` reports `replicated` correctly.
