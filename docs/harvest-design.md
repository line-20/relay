...
The E2 gap — the checkpoint "lived on-disk in the worktree, resolvable but uncommitted" — is closed. `apply_pending` now, when given a repo and replication is on, commits each newly-applied checkpoint (with its completion marker, projection handover, and the board) to the **durable branch** and pushes it (`_replicate_checkpoint`, `scripts/relay_harvest.py`). Local commit is the offline-safe floor; the push is best-effort and **never fails the apply** (R1.6's local-vs-remote split). A per-item `replicated` view (`_checkpoint_on_remote`, blob-hash comparison against the local `origin/<durable>` ref — no network) is surfaced through `discover_active`, so the maintainer can see which threads are resumable from another device.

**Ref choice — durable branch, not a topic branch or a dedicated ref** (challenger-ruled, `relay/decisions.md` 2026-09-17): the rediscover/resume readers walk the *checked-out working tree* (`discover_active` / `_all_item_slugs` read `<root>/harvest/*/checkpoint.json` off the filesystem), so a fresh clone — or a phone — only finds a thread when its checkpoint lives in the durable branch's working tree, where board/handover already commit (R1.10). A topic branch scatters checkpoints across refs the main checkout can't see and is deleted on merge; a `refs/relay/*` ref is fetched by no default clone and sits in no working tree. `_replicate_checkpoint` therefore **refuses to commit onto anything but the durable branch** (the case-B trap guard). **Cadence:** push per-checkpoint (opportunistic), not throttled to the session boundary — once origin is the location that matters for cross-device resume, a once-per-session push leaves origin stale mid-session, reopening the loss window (R1.5 wants a recent durable checkpoint to always exist).

**Wired into the happy path (slice 4).** `/handover` Step 4 now emits the session's resume state as a harvest result and runs `apply` (with replication) against the **main checkout** *before* it commits the handover, so a normal handover — and therefore `/ship`, which calls `/handover` — leaves the thread's checkpoint on `origin/main` automatically. Because the runtime commits on the durable branch's working tree, `/handover` resolves the main checkout from `git worktree list` and points `apply --repo` at it (all git calls are `cwd=repo_root`, so the caller's worktree is irrelevant). The checkpoint goes first — the Markdown handover is its projection — but a checkpoint that can't replicate (offline, or the main checkout is off the durable branch) **never blocks the handover push**: resume degrades to the Markdown handover exactly as before, and the outcome is surfaced on the handover summary's `Resume:` line. `/handover` passes `--no-projection` (it authors its own richer handover, so main never advertises the thin projection) and `--no-board` (Step 4b owns the board commit, avoiding two competing board writes per lap) — challenger-ruled Option A with these build refinements, `relay/decisions.md` 2026-09-17. The write-ahead result (`<root>/harvest/*/results/`) is transient staging consumed by `apply`, so it is gitignored; only the checkpoint it produces is durable.

**Proven by** `CheckpointDurabilityTest` (`scripts/tests/test_relay_harvest.py`, real bare origin): recoverable from git after the worktree is removed; resumable from a fresh clone (the cross-device proof); resumable from a fresh clone when `apply` is driven against a main checkout while the work sits in a *separate* worktree (the `/handover` invocation shape); offline push is non-fatal (local floor holds); replication refuses a topic branch; idempotent re-apply makes no second commit; re-emit of the same lap stages no duplicate; `--no-replicate` writes state only; `--no-projection` writes no projection handover yet still replicates the checkpoint; `discover_active` reports `replicated` correctly.
