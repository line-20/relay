...
Built in `plugins/relay/bin/relay_harvest.py` (+ `scripts/tests/test_relay_harvest.py`). What implementation confirmed or adjusted vs. the design above:
...
- **What it does NOT prove:** that a foreign worker can emit the result itself under a locked-down sandbox (the D limit above — an environment fix, not a contract one); anything about a real multi-provider *runtime/supervisor* (orchestration was still a manual harness); code-level continuation across providers (Codex's bounded step was a decision doc, deliberately not core code); git-durability of the checkpoint (~~it lived on-disk in the worktree, resolvable but uncommitted — a hardening, not proven~~ — **closed, see R1.12**); and nothing about Organisation scope.

## R1.12 — Checkpoint git-durability (slice 3, built)

The E2 gap — the checkpoint "lived on-disk in the worktree, resolvable but uncommitted" — is closed. `apply_pending` now, when given a repo and replication is on, commits each newly-applied checkpoint (with its completion marker, projection handover, and the board) to the **durable branch** and pushes it (`_replicate_checkpoint`, `plugins/relay/bin/relay_harvest.py`). The on-disk checkpoint is the offline floor; the push is best-effort and **never fails the apply** (R1.6's local-vs-remote split). A per-item `replicated` view (`_checkpoint_on_remote`, blob-hash comparison against the local `origin/<durable>` ref — no network) is surfaced through `discover_active`, so the maintainer can see which threads are resumable from another device.

**Commit mechanism — parallel-worktree-safe temp index** (slice 4, after review): `_replicate_checkpoint` builds the checkpoint commit the same way `/handover` Step 4b and `/tidy` write `main` — read the freshly-fetched durable tip into a throwaway index (`GIT_INDEX_FILE`), stage only the checkpoint files into it, `write-tree`, `commit-tree` onto that tip, and push `<commit>:<durable>`. It **never touches repo_root's working tree, index, HEAD or local branch**, so it is safe while ~10 sessions run concurrent worktrees against one checkout (`docs/conventions.md` "Parallel-worktree-safe commits") and it runs correctly from **any** checkout — the slice worktree included, so `/handover` no longer reaches into the main checkout. Because the parent is the fetched remote tip (never repo_root's possibly-stale local branch), the push fast-forwards whenever the remote hasn't moved since the fetch; a sibling advancing the remote in between is a non-ff that the *next* apply rebuilds-and-retries from a fresh fetch (self-heal), not a permanent rejection. The earlier working-tree-commit implementation is retired: it moved a shared checkout's HEAD, contended on its `index.lock`, and — because Step 4b advances `origin/main` without advancing any local `main` — left every checkpoint after the first a permanent non-fast-forward. There is consequently **no branch guard** (the commit lands on the durable branch by construction).

**Ref choice — durable branch, not a topic branch or a dedicated ref** (challenger-ruled, `relay/decisions.md` 2026-09-17): the rediscover/resume readers walk the *checked-out working tree* (`discover_active` / `_all_item_slugs` read `<root>/harvest/*/checkpoint.json` off the filesystem), so a fresh clone — or a phone — only finds a thread when its checkpoint lives in the durable branch's working tree, where board/handover already commit (R1.10). A topic branch scatters checkpoints across refs the main checkout can't see and is deleted on merge; a `refs/relay/*` ref is fetched by no default clone and sits in no working tree. The temp-index `commit-tree`-onto-fetched-tip parents on the durable branch by construction, so the checkpoint always lands there. **Cadence:** push per-checkpoint (opportunistic), not throttled to the session boundary — once origin is the location that matters for cross-device resume, a once-per-session push leaves origin stale mid-session, reopening the loss window (R1.5 wants a recent durable checkpoint to always exist).

**Wired into the happy path (slice 4).** `/handover` Step 4 now emits the session's resume state as a harvest result and runs `apply` (with replication) **in the slice worktree** *before* it commits the handover, so a normal handover — and therefore `/ship`, which calls `/handover` — leaves the thread's checkpoint on `origin/main` automatically. The temp-index commit mechanism (above) makes reaching into the main checkout unnecessary. The checkpoint goes first — the Markdown handover is its projection — but a checkpoint that can't replicate (offline, or a sibling advanced the remote between the fetch and the push) **never blocks the handover push**: nothing local changed, resume degrades to the Markdown handover exactly as before, the next lap self-heals, and the outcome is surfaced on the handover summary's `Resume:` line. The result JSON is written with the **Write tool, never interpolated into the shell** — its free-text fields (next objective, scope edges, open questions) are handover prose that could otherwise trigger command substitution before an automatic push. `/handover` passes `--no-projection` (it authors its own richer handover, so main never advertises the thin projection) and `--no-board` (Step 4b owns the board commit, avoiding two competing board writes per lap) — challenger-ruled Option A, with the temp-index mechanism and these refinements settled after the slice-4 review, `relay/decisions.md` 2026-09-17. The write-ahead result (`<root>/harvest/*/results/`) is transient staging consumed by `apply`, so it is gitignored; only the checkpoint it produces is durable. Provider-neutrality is enforced by a **recursive** scan (`_scan_provider_leak`) — a `session_id`/`provider` nested anywhere in `references`/`resume_delta` is rejected before it can be copied into the pushed checkpoint.

**Proven by** `CheckpointDurabilityTest` (`scripts/tests/test_relay_harvest.py`, real bare origin): recoverable from origin after the worktree is removed; resumable from a fresh clone (the cross-device proof); **two consecutive handovers both replicate even when the remote is advanced independently between them** (the non-fast-forward regression the review caught); replication works from a checkout on a topic branch and still lands on the durable branch, touching neither the topic branch nor local `main` (the parallel-worktree-safe property); a sibling's staged file is never swept into the pushed commit; offline push is non-fatal (on-disk floor holds, apply still succeeds); idempotent re-apply makes no second commit; re-emit of the same lap stages no duplicate; `--no-replicate` writes state only; `--no-projection` writes no projection yet still replicates; the CLI `main()` flag mapping is exercised directly; a nested provider field is rejected; `discover_active` reports `replicated` correctly.
