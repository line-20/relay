...
Relay has two required durable scopes — **Project → Work Item** (an Organisation context above Project is external and out of scope). A durable worker reference must be resolvable within those scopes. Cross-provider validation forced two clarifications, both fixed at the **bootstrap layer** (no checkpoint-schema change):
